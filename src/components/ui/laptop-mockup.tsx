      ...
      <div className="grid grid-cols-3 gap-1 h-[calc(100%-16px)] relative z-10">
        ...
        <div className="bg-white/5 rounded p-1 space-y-1">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className={`h-1.5 rounded-sm w-3/4 ${i === 1 ? 'bg-blue-500' : 'bg-white/10'}`}
            />
          ))}
        </div>
        {/* Main Content */}
        <div className="col-span-2 space-y-1">
          ...
          <div className="grid grid-cols-3 gap-1">
            {['blue', 'violet', 'emerald'].map((color, i) => (
              <div
                key={i}
                className={`bg-${color}-500/20 rounded p-1 border border-${color}-500/20`}
              >
        ...
        <div className="flex justify-start">
          <div className="bg-zinc-800 rounded-lg px-2 py-1">
            <div className="flex gap-0.5">
              ...
              <div
                className="w-0.5 h-0.5 rounded-full bg-emerald-400 animate-pulse"
      ...
      return (
        <>
          <motion.div
            variants={floatAnim}
            custom={1}
            className="absolute -right-4 top-0 bg-blue-500 rounded-lg p-2 shadow-xl border border-white/10"
            style={{ transform: 'translateZ(40px) rotateY(-10deg) rotateX(5deg)' }}
          >
          ...
          <motion.div
            variants={floatAnim}
            custom={2}
            className="absolute -left-6 bottom-8 bg-black/80 backdrop-blur rounded-lg p-2 shadow-xl border border-white/10"
            style={{ transform: 'translateZ(60px) rotateY(15deg)' }}
          >
            <div className="flex gap-1">
    ...
    case 'ai-agents':
      return (
        <>
          <motion.div
            variants={floatAnim}
            className="absolute -right-5 top-4 bg-emerald-500 rounded-xl p-2 shadow-lg border border-white/10"
            style={{ transform: 'translateZ(50px)' }}
          >
            <div className="w-4 h-4 text-black font-bold flex items-center justify-center text-[10px]">
              AI
            </div>
          </motion.div>
          <motion.div
            variants={floatAnim}
            className="absolute -left-2 top-[-20px] bg-zinc-800 rounded-lg py-1 px-2 border border-white/10"
    ...
    case 'branding':
      return (
        <>
          <motion.div
            variants={floatAnim}
            className="absolute -right-4 bottom-10 bg-gradient-to-br from-pink-500 to-rose-600 rounded-lg w-8 h-10 shadow-xl border border-white/10 -rotate-6"
            style={{ transform: 'translateZ(40px)' }}
          />
          <motion.div
            variants={floatAnim}
            className="absolute -right-8 bottom-6 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-lg w-8 h-10 shadow-xl border border-white/10 rotate-12"
            style={{ transform: 'translateZ(20px)' }}
          />
          <motion.div
            variants={floatAnim}
            className="absolute -left-4 top-0 bg-black/90 text-white rounded px-2 py-0.5 text-[8px] font-bold border border-white/20"
    ...
    case 'automation':
      return (
        <>
          <motion.div
            variants={floatAnim}
            className="absolute right-0 -top-6 bg-orange-500 rounded-full p-1.5 shadow-lg border border-white/10"
            style={{ transform: 'translateZ(40px)' }}
          >
          ...
          <motion.div
            variants={floatAnim}
            className="absolute -left-4 bottom-4 bg-zinc-800 rounded p-1.5 shadow-xl border border-white/10"
        ...
        >
          ...
          >
            ...
            <div className="w-full h-full bg-black overflow-hidden rounded relative border border-white/5">
              {screenImage ? (
                <Image src={screenImage} alt="Screen content" fill className="object-cover" />
              ) : screenContent ? (
              ...
              >
                ...
                <div className="grid grid-cols-[repeat(14,1fr)] gap-[2px]">
                  {Array.from({ length: 14 }).map((_, i) => (
                    <div
                      key={`r1-${i}`}
                      className="bg-[#111] rounded-[1px] h-full shadow-[0_1px_0_rgba(255,255,255,0.1)]"
                    />
                  ))}
                </div>
                {/* Row 2 */}
                <div className="grid grid-cols-[1.5fr_repeat(12,1fr)_1.5fr] gap-[2px]">
                  {Array.from({ length: 14 }).map((_, i) => (
                    <div
                      key={`r2-${i}`}
                      className="bg-[#111] rounded-[2px] h-full shadow-[0_1px_0_rgba(255,255,255,0.1)]"
                    />
                  ))}
                </div>
                {/* Row 3 */}
        ...
        >
          <CardContainer className="inter-var w-full">
            <CardBody className="liquid-glass-card relative group/card w-full h-auto rounded-3xl p-8 border-white/10 hover:border-white/20">
              {/* Background Gradient */}
              <CardItem translateZ="50" className="absolute inset-0 -z-10">
                <div
                  className={`w-full h-full rounded-3xl bg-gradient-to-br ${service.gradient} opacity-0 group-hover/card:opacity-100 transition-opacity duration-500`}
                />
              </CardItem>
              ...
              {/* Title & Context */}
              <CardItem translateZ="60" className="relative z-10 mb-8 w-full">
              ...
              <CardItem translateZ="100" rotateX={10} rotateY={-10} className="w-full mt-8">
