...
export default function Testimonials({ cards }: TestimonialsProps) {
  ...
  const getCardClassName = (index: number, baseClassName: string) => {
    ...
    if (focusedIndex === 0 && index === 1) {
      return (
        baseClassName + ' !translate-y-20 sm:!translate-y-32 !translate-x-14 sm:!translate-x-24'
      );
    }
    if (focusedIndex === 0 && index === 2) {
      return (
        baseClassName + ' !translate-y-28 sm:!translate-y-44 !translate-x-24 sm:!translate-x-40'
      );
    }
    if (focusedIndex === 1 && index === 2) {
      return (
        baseClassName + ' !translate-y-24 sm:!translate-y-40 !translate-x-24 sm:!translate-x-40'
      );
    }
  ...
  const defaultCards: TestimonialCardProps[] = [
    {
      ...
      handle: '@sarahchen',
      content:
        'This component is exactly what I needed for my landing page. The stacked effect is beautiful! 🎨',
      date: 'Jan 3, 2026',
      verified: true,
      likes: 42,
      retweets: 8,
    ...
    },
    {
      ...
      handle: '@mikej_dev',
      content:
        'The hover interactions are so smooth. Love how the cards spread apart to reveal the ones behind. Great UX thinking!',
      date: 'Jan 2, 2026',
      verified: true,
      likes: 28,
      retweets: 5,
